...
class AndrewProV63StrategySeederTest extends TestCase
{
    use RefreshDatabase;

    public function test_seeds_andrew_strategy_preset_without_bots(): void
    {
        Artisan::call('db:seed', ['--class' => AndrewProV63StrategySeeder::class]);

        $strategy = Strategy::query()
            ->where('name', AndrewProV63StrategySeeder::STRATEGY_NAME)
            ->firstOrFail();
        ...
        $this->assertSame(60, $entry->interval);
        $this->assertSame(1000, $entry->kline_limit);
        $this->assertSame(25.0, $entry->adx_min);
        $this->assertSame(2.5, $risk->tp_multiplier);
        $this->assertSame(0.01, $risk->max_risk_per_trade);
        $this->assertSame(2.0, $risk->daily_profit_target_pct);
